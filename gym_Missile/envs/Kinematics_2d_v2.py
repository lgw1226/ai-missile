...
class Kinematics2d_Env_v2(gym.Env):
    ...
    def __init__(self, **kwargs: dict[str, float]):
        ...
        '''
        Head-on scenario
        Missile : x, z, theta, a_z
        Target  : x, z, theta, a_z
        '''
        self.state_dim                  = 4
        self.obs_dim                    = 5
        self.act_dim                    = 1
        self.M_V                        = kwargs.get('missile_speed', 1000)
        self.T_V                        = kwargs.get('target_speed', 500)
        self.grav                       = 9.806
    ...
    def reset(self, options={}):
        ...
        # Learning ====================================
        # self.tau                        = np.random.randint(1,4,1) * 0.1  # 0.1 ~ 0.3
        # self.M_states[0]                = 0
        # self.M_states[1]                = 0
        ...
        self.T_states[0]                = 2e3
        self.T_states[1]                = 0
        self.T_states[2]                = np.random.randint(0, 181, 1) * D2R
        self.T_states[3]                = 0

        # if option is given, set initial conditions as following
        self.tau = options.get('tau', self.tau)
        self.M_states[2] = options.get('theta_m', self.M_states[2] / D2R) * D2R
    ...
    def get_diff(self, action, Mstates, Tstates):
        ...
        M_theta_dot       = M_a_z/M_V
        # M_theta_dot       = (action - self.grav*cos(M_theta))/V_M
        M_a_z_dot         = - (1 / self.tau) * M_a_z + (1 / self.tau) * action
        ...
        _Mstates[3]    = M_a_z_dot 
        
        _Tstates[0]    = T_x_dot 
        _Tstates[1]    = T_z_dot 
        _Tstates[2]    = T_theta_dot 
        _Tstates[3]    = T_a_z_dot 
    ...
    def get_obs(self, Mstates, Tstates, initial_range):
        ...
        NN_input[0] = np.clip(LOS, -np.pi, np.pi)
        NN_input[1] = np.clip(LOS_dot, -np.pi, np.pi)
        NN_input[2] = Range / initial_range
        NN_input[3] = np.clip(Range_dot / 1000, -1, 1)
        NN_input[4] = self.tau
    ...
    def step(self, action):
        ...
        # saturation a_z
        next_Mstates[3] = max(min(next_Mstates[3], self.control_gain), -self.control_gain)
        ...
        if success or (self.count > self.max_episode_length) or (Range > prev_Range):
            done = True
            self.episode += 1
