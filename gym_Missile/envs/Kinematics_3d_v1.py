...
class Kinematics3d_Env_v1(gym.Env):
    metadata = {'render.modes': ['human']}
    
    def __init__(self):
        super(Kinematics3d_Env_v1, self).__init__()
        self.reward                     = 0
        self.episode                    = 0
        self.success_epi                = 0
        self.noise                      = 0.001
    ...
    def get_reward(self ,Mstates, Tstates, prev_Mstates, prev_Tstates):
        ...
        # scale range_dot before returning values
        return reward, (np.clip(LOS_g_dot, -np.pi/2, np.pi), np.clip(LOS_t_dot, -np.pi/2, np.pi/2))
    
    def get_obs(self, Mstates: np.ndarray, Tstates, initial_Range):
        # convert euler angles to quaternion
        # or we can try using cosine and sine pairs to represent the angles
        # eul = Mstates[6:]
        # rot = Rotation.from_euler('xyz', eul)
        # quat = rot.as_quat(canonical=True)

        # observation = np.zeros((self.obs_dim,), dtype=np.float32)
        # observation[:6] = Mstates[:6] / 1000  # scale position and speed
        # observation[6:] = quat
        ...
        rotation = Rotation.from_euler('xyz', [phi_M, theta_M, psi_M])
        quat = rotation.as_quat(True)

        rotation_t = Rotation.from_euler('xyz', [phi_T, theta_T, psi_T])
        quat_t = rotation_t.as_quat(True)
